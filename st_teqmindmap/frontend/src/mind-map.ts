...
export class MindMap extends LitElement {
  ...
  @query('#graph')
  _graph!: HTMLDivElement


  changeData(d: any, level?: number, color?: string) {
    const data = {
      ...d,
    };
    ...
    if (d.children) {
      data.children = d.children.map((child: any) => this.changeData(child, (level ? level + 1 : 1), data.color));
    }
    return data;
  };

  dataTransform(data: object) {
  ...
  firstUpdated() {
    ...
    console.log(this.dataTransform(this.rawData));
    ...
    G6.registerNode(
      ...
      jsx: (cfg) => {
        ...
        const stroke = (cfg.style && cfg.style.stroke) || '#096dd9';
        // const fill = (cfg.style && cfg.style.fill);
    ...
    G6.registerNode(
      ...
      jsx: (cfg) => {
        ...
        const color = cfg.color || (cfg.style && cfg.style.stroke);
    ...
    G6.registerBehavior('scroll-canvas', {
      ...
      onWheel: function onWheel(ev: any) {
        ...
        if (!graph || !(graph instanceof G6.Graph)) {
          return;
        }
        if (ev.ctrlKey) {
    ...
    const width = this._graph.scrollWidth;
    const height = (this._graph.scrollHeight || 500) - 20;
    const tree = new G6.TreeGraph({
      ...
      minZoom: 0.5,
      modes: {
        default: ['drag-node', 'drag-canvas', 'zoom-canvas', 'dice-mindmap'],
      },
    });
